...
## Texture2D format handling update

The Unity 6000.3.12f1 retarget pipeline now treats `m_TextureFormat == 3` (`RGB24`) as a source format that must be converted to RGBA32. `RGBA32` (4) and RGBA ASTC 6x6 (50) remain fast-path formats and are skipped entirely. RGB24 decoding already expands each pixel with an opaque alpha channel before the existing RGBA32 rebuild path.
