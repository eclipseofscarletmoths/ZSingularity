...
+ (BOOL)resolveSymbols {
    ...
    // String helpers are resolved separately and NOT included in the
    // g_resolveOk gate below - everything else in this bridge (the
    // graphics/render path) has never needed a String return value, so
    // one exotic Unity build missing these two exports shouldn't take
    // the whole bridge down. nsStringFromIl2CppString: just fails soft
    // (returns nil) if they're unavailable.
    ...
    // Same "optional, not part of g_resolveOk" treatment as the string
    // helpers above - see the header for why. Only
    // gd_install_scene_loaded_hook (GDScripts.m) touches these.
...
+ (void *)invokeMethod:(const void *)method
    ...
    // Deliberately NOT logged on the success path - this is called on
    // every settings apply/slider drag, easily dozens of times a second,
    // and ZLog-ing each one would flood the Verbose console and add
    // real per-call overhead. An IL2CPP exception is rare and always
    // worth surfacing, so that's the only outcome logged here.
    if (exc) {
...
+ (NSString *)nsStringFromIl2CppString:(void *)il2cppString {
    ...
    if (!chars) return nil;
    // IL2CPP strings are UTF-16 (like .NET's), same as NSString's
    // native encoding, so this is a direct byte-length copy rather
    // than a real transcode.
